...
public class CTLR {
	...
	// topic-word counts
	public int[][] n_wz = null; // n_wz[z][w]: number of times word w is
	...
	public CTLR(String _datasetPath, int _nTopics, int _batch) {
		...
		n_wz = new int[nTopics][dataset.vocabulary.length];
			...
			for (int s = 0; s < currUser.nPosts; s++) {
				if (currUser.postBatches[s] == batch) {
					...
					for (int w = 0; w < currPost.words.length; w++) {
						...
						postWordLikelihood += topicWordDist[currPost.topic][word];
					}
	...
	private void altOptimize_topics() {
		...
		for (int k = 0; k < nTopics; k++) {
			for (int w = 0; w < dataset.vocabulary.length; w++) {
				topicWordDist[k][w] = (n_wz[k][w] + gamma) / (sum_nwz[k] + (gamma * dataset.vocabulary.length));
			}
		}
	}
	...
	public void train() {
		...
		for (int iter = 0; iter < max_GibbsEM_Iterations; iter++) {
			...
			System.out.println();
			for (int k=0;k<nTopics;k++){
				System.out.printf("%f \t", dataset.users[0].authorities[k]);
			}
			System.out.println();
			for (int k=0;k<nTopics;k++){
				System.out.printf("%f \t", dataset.users[0].hubs[k]);
			}
			System.out.println();
			*/
	...
	private void outputPostTopicTopWords(int k) {
		try {
			File f = new File(dataset.path + "/l_topTopicWords.csv");
