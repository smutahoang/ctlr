...
public class MultithreadCTLR {
	...
	// topic-word counts
	private int[][] n_zw = null; // n_wz[z][w]: number of times word w is
									// generated by topic z in a post
	private int[] sum_nzw = null; // sum_nwz[z]: total number of times words
									// that
	...
	public static int max_Gibbs_Iterations = 600;
	...
	/****
	 * initialize for user u
	 * 
	 * @param u
	 */
	private static void init(int u) {
		...
		if (!initByTopicModeling) {
			for (int k = 0; k < nTopics; k++) {
				currUser.topicalInterests[k] = (n_zu[k][u] + alpha) / (dataset.users[u].nPosts + (alpha * nTopics));
	...
	private double getOptPostLikelihood_TwitterLDA(int u, int j) {
